  ...
  if (loading || !stats) {
    return (
      <div className="py-20 text-center">
        ...
        <p className="mt-4 text-slate-400 text-sm">Loading executive admin portal...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-8 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <span className="text-xs font-bold uppercase tracking-wider text-purple-400">Executive Control Panel (India)</span>
          <h1 className="text-3xl font-extrabold text-white mt-1">
            Platform Operations & Revenue Overview
          </h1>
          <p className="text-slate-400 text-sm mt-1">Real-time marketplace monitoring, MongoDB Atlas data, and Rupee (₹) revenue analytics.</p>
        </div>
      </div>
          ...
          <div className="space-y-4 pt-2">
            <div className="space-y-1.5">
              <div className="flex justify-between text-xs font-semibold">
                ...
                <span className="text-teal-400">₹{kpis.booking_revenue?.toFixed(2)}</span>
              </div>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between text-xs font-semibold">
                ...
                <span className="text-amber-400">₹{kpis.subscription_revenue?.toFixed(2)}</span>
              </div>
              <div className="w-full bg-slate-950 rounded-full h-2.5 overflow-hidden">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-8 shadow-xl space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
          <div>
            ...
            <p className="text-xs text-slate-400">Manage platform permissions & toggle premium tiers</p>
          </div>
                ...
                <tr key={u.id} className="hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="py-3.5 px-4 text-xs">
                    {u.is_premium ? (
                      <span className="text-amber-400 font-bold flex items-center gap-1">
                        <Crown className="w-3.5 h-3.5 fill-amber-400" /> Premium Member
                      </span>
                    ) : (
                  ...
                  <td className="py-3.5 px-4 text-right">
                    <button
                      ...
                      className="px-3 py-1 rounded bg-slate-800 hover:bg-slate-700 text-xs font-semibold text-white border border-slate-700"
                    >
                      {u.is_premium ? 'Demote to Standard' : 'Grant Premium Pass'}
                    </button>
                  </td>
                </tr>
              ))}
